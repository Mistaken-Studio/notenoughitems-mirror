...
namespace Mistaken.NotEnoughItems.Items
{
    ...
    {
        internal static readonly Dictionary<GameObject, Door> Doors = new();

        internal static readonly HashSet<ItemType> UsableItems = new()
        {
            ItemType.MicroHID,
            ItemType.Medkit,
        ...
        internal static readonly Vector3 Size = new(1f, 0.65f, 1f);
        ...
        public override void Give(Player player, bool displayMessage = true)
        {
            ...
            firearm.NetworkStatus = new FirearmStatus(ClipSize, FirearmStatusFlags.Cocked, 75);
            player.AddItem(pickup);
            RLogger.Log("TASER", "GIVE", $"{Name} given to {player.PlayerToString()}");
        ...
        public override void Give(Player player, Pickup pickup, bool displayMessage = true)
        {
            ...
            firearm.NetworkStatus = new FirearmStatus(ClipSize, FirearmStatusFlags.Cocked, 75);
            player.AddItem(pickup);
            RLogger.Log("TASER", "GIVE", $"Given {Name} to {player.PlayerToString()}");
        ...
        public override Pickup Spawn(Vector3 position, Item item, Player previousOwner = null)
        {
            ...
            TrackedSerials.Add(pickup.Serial);
            ((FirearmPickup)pickup.Base).Status = new FirearmStatus(ClipSize, FirearmStatusFlags.Cocked, 75);
            return pickup;
        }
        ...
        protected override void SubscribeEvents()
        {
            ...
            Server.RoundStarted += Server_RoundStarted;
        ...
        {
            Module.RunSafeCoroutine(UpdateInterface(player), "Taser.UpdateInterface");
        }
        ...
        {
            RLogger.Log("TASER", "PICKUP", $"{player.PlayerToString()} Picked up an {Name}");
            player.SetGUI("taserpickedupmessage", PseudoGUIPosition.MIDDLE,
                string.Format(PluginHandler.Instance.Translation.ItemPickedUpMessage,
                    PluginHandler.Instance.Translation.Taser), 2f);
        }
        ...
        protected override void OnShooting(ShootingEventArgs ev)
        {
            ...
            {
                isShotAllowed = false;
                ev.Shooter.SetGUI("taserammo", PseudoGUIPosition.TOP, PluginHandler.Instance.Translation.TaserNoAmmo,
                    2);
                ev.IsAllowed = false;
                return;
            }

            (ev.Shooter.CurrentItem as Firearm).Ammo += 1;
            cooldowns[ev.Shooter.CurrentItem.Serial] =
                DateTime.Now.AddSeconds(PluginHandler.Instance.Config.HitCooldown);
            var targetPlayer = RealPlayers.List.Where(x => x.NetworkIdentity.netId == ev.TargetNetId).Count() > 0
                ? RealPlayers.List.First(x => x.NetworkIdentity.netId == ev.TargetNetId)
                : null;
            if (targetPlayer != null)
            {
                Hitmarker.SendHitmarker(ev.Shooter.Connection, 20);
                if (targetPlayer.Items.Select(x => x.Type).Any(x =>
                        x == ItemType.ArmorLight || x == ItemType.ArmorCombat || x == ItemType.ArmorHeavy))
                {
                    RLogger.Log("TASER", "BLOCKED",
                        $"{ev.Shooter.PlayerToString()} hit {targetPlayer.PlayerToString()} but effects were blocked by an armor");
                    return;
                }

                if (targetPlayer.GetSessionVariable<bool>(SessionVarType.SPAWN_PROTECT))
                {
                    RLogger.Log("TASER", "REVERSED",
                        $"{ev.Shooter.PlayerToString()} hit {targetPlayer.PlayerToString()} but effects were reversed because of spawn protect");
                ...
                if (targetPlayer.IsHuman)
                {
                    ...
                    targetPlayer.EnableEffect<Amnesia>(5);
                    ...
                    targetPlayer.Broadcast($"<color=yellow>{PluginHandler.Instance.Translation.Taser}</color>", 10,
                        string.Format(PluginHandler.Instance.Translation.TaserPlayerTased, ev.Shooter.Nickname,
                            ev.Shooter.Role));
                    targetPlayer.SendConsoleMessage(
                        string.Format(PluginHandler.Instance.Translation.TaserPlayerTased, ev.Shooter.Nickname,
                            ev.Shooter.Role), "yellow");
                }
            }
            else
            {
                ...
                {
                    Log.Debug($"TaserItem Debug: {hitinfo.collider.name}",
                        PluginHandler.Instance.Config.VerbouseOutput);
                    if (!Doors.TryGetValue(hitinfo.collider.gameObject, out var door) || door == null)
                    {
                        ...
                        cooldowns[ev.Shooter.CurrentItem.Serial] =
                            DateTime.Now.AddSeconds(PluginHandler.Instance.Config.MissCooldown);
                        return;
                    }
        ...
        private void Server_RoundStarted()
        {
            ...
            var structureLockers = Object.FindObjectsOfType<SpawnableStructure>()
                .Where(x => x.StructureType == StructureType.LargeGunLocker);
        ...
        private IEnumerator<float> UpdateInterface(Player player)
        {
            ...
            while (Check(player.CurrentItem))
            {
                ...
                var diff = (PluginHandler.Instance.Config.HitCooldown - (time - DateTime.Now).TotalSeconds) /
                    PluginHandler.Instance.Config.HitCooldown * 100;
                ...
                player.SetGUI("taserholding", PseudoGUIPosition.BOTTOM,
                    string.Format(PluginHandler.Instance.Translation.TaserHold, bar));
