...
namespace Mistaken.NotEnoughItems.Items
{
    ...
    {
        internal static readonly Vector3 Size = new(2, 2, 2);
        ...
        public override void Give(Player player, bool displayMessage)
        {
            ...
            firearm.NetworkStatus = new FirearmStatus(ClipSize, FirearmStatusFlags.MagazineInserted, 594);
            player.AddItem(pickup);
            RLogger.Log("MEDIC GUN", "GIVE", $"Given {Name} to {player.PlayerToString()}");
        ...
        {
            var firearm = (FirearmPickup)pickup.Base;
            firearm.NetworkStatus = new FirearmStatus(ClipSize, FirearmStatusFlags.Cocked, 594);
            player.AddItem(pickup);
            RLogger.Log("MEDIC GUN", "GIVE", $"Given {Name} to {player.PlayerToString()}");
        ...
        public override Pickup Spawn(Vector3 position, Item item, Player previousOwner = null)
        {
            ...
            RLogger.Log("MEDIC GUN", "SPAWN", $"Spawned {Name}");

            pickup.Scale = Size;
            TrackedSerials.Add(pickup.Serial);
            ((FirearmPickup)pickup.Base).Status = new FirearmStatus(ClipSize, FirearmStatusFlags.Cocked, 594);
            return pickup;
        }
        ...
        {
            RLogger.Log("MEDIC GUN", "PICKUP", $"{player.PlayerToString()} Picked up an {Name}");
            player.SetGUI("medicgunpickedupmessage", PseudoGUIPosition.MIDDLE,
                string.Format(PluginHandler.Instance.Translation.ItemPickedUpMessage,
                    PluginHandler.Instance.Translation.MedicGun), 2f);
        }
        ...
        protected override void OnReloading(ReloadingWeaponEventArgs ev)
        {
            base.OnReloading(ev);
            ev.IsAllowed = false;
            if (ev.Firearm.Ammo >= ClipSize)
            {
                ev.Player.SetGUI("MedicGunWarn", PseudoGUIPosition.BOTTOM,
            ...
            if (!ev.Player.Items.Any(i => i.Type == ItemType.Adrenaline))
            {
                ev.Player.SetGUI("MedicGunWarn", PseudoGUIPosition.BOTTOM,
                    string.Format(PluginHandler.Instance.Translation.NoAmmoError,
                        PluginHandler.Instance.Translation.MedicGunAmmo), 3);
                return;
            }
            ...
            RLogger.Log("MEDIC GUN", "RELOAD", $"Player {ev.Player.PlayerToString()} reloaded {Name}");
            ev.Player.RemoveItem(ev.Player.Items.First(i => i.Type == ItemType.Adrenaline));
            ev.Player.SetGUI("MedicGunWarn", PseudoGUIPosition.BOTTOM, PluginHandler.Instance.Translation.ReloadedInfo,
                3);
        ...
        protected override void OnShot(ShotEventArgs ev)
        {
            ...
            {
                var hpToHeal = Math.Min(ev.Target.MaxHealth - ev.Target.Health,
                    PluginHandler.Instance.Config.HealAmount);
                var ahpToHeal = (PluginHandler.Instance.Config.HealAmount - hpToHeal) * 2f;
                ev.Target.Health += hpToHeal;
                if (Math.Floor(ahpToHeal) != 0)
                    ((AhpStat)ev.Target.ReferenceHub.playerStats.StatModules[1]).ServerAddProcess(ahpToHeal, ahpToHeal,
                        ahpToHeal / 10f, 0.65f, 15f, false);
                RLogger.Log("MEDIC GUN", "HEAL",
                    $"Player {ev.Shooter.PlayerToString()} hit player {ev.Target.PlayerToString()} and regenerated {hpToHeal} hp and {ahpToHeal} ahp");
                ev.CanHurt = false;
