...
namespace Mistaken.NotEnoughItems.Items
{
    ...
    {
        internal static readonly Vector3 Size = new(1f, 0.4f, 1f);
        ...
        {
            RLogger.Log("IMPACT GRENADE", "PICKING UP", $"{ev.Player.Nickname} started picking up {Name} ({Type})");
            base.OnPickingUp(ev);
        }
        ...
        {
            RLogger.Log("IMPACT GRENADE", "ACQUIRED", $"{player.Nickname} acquired {Name} ({Type})");
        ...
        {
            RLogger.Log("IMPACT GRENADE", "GIVE", $"{Name} given to {player.PlayerToString()}");
        ...
        public override Pickup Spawn(Vector3 position, Item item, Player previousOwner = null)
        {
            ...
            RLogger.Log("IMPACT GRENADE", "SPAWN", $"{Name} spawned");
        ...
        {
            RLogger.Log("IMPACT GRENADE", "PICKUP", $"{player.PlayerToString()} Picked up an {Name}");
            player.SetGUI("imppickedupmessage", PseudoGUIPosition.MIDDLE,
                string.Format(PluginHandler.Instance.Translation.ItemPickedUpMessage,
                    PluginHandler.Instance.Translation.ImpactGrenade), 2f);
        }
        ...
        protected override void OnThrowing(ThrowingItemEventArgs ev)
        {
            ...
            {
                RLogger.Log("IMPACT GRENADE", "THROW", $"{ev.Player.PlayerToString()} threw an {Name}");
        ...
        protected override void OnExploding(ExplodingGrenadeEventArgs ev)
        {
            ...
            foreach (var player in ev.TargetsToAffect)
                RLogger.Log("IMPACT GRENADE", "HURT", $"{player.PlayerToString()} was hurt by an {Name}");
        }
